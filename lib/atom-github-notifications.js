...
export function fetch() {
  ...
  const onlyParticipating = atom.config.get('atom-github-notifications.showOnlyDirectParticipation');
    ...
    .then(([...notifications]) => Promise.all(
      R.map(
        (notification) => {
          ...
          if (commentId) {
            const fetchFunction = type === NotificationSubjectTypes.COMMIT
              ? R.bind(github.repos.getCommitCommentAsync, github.repos)
              : R.bind(github.issues.getCommentAsync, github.issues);
