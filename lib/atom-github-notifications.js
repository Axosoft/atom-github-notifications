...
function getNotificationMessage(type, userLogin) {
  const subjectDisplayName = SubjectTypeDisplayNames[type];
  const userString = userLogin ? ` by @${userLogin}` : '';
  return `You were mentioned in a ${subjectDisplayName}${userString}`;
}
...
function showNotification({ title, body, userLogin, subjectId, subjectUrl, subjectType }) {
  atom.notifications.addInfo(getNotificationMessage(subjectType, userLogin), {
    dismissable: true,
